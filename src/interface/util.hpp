...
template<typename K, typename V, typename H, typename E> using Mymap = std::unordered_map<K, V, H, E>;
...
namespace util {
  ...
  template<typename T> Float adjustCnfModelCount(Float apparentModelCount, const T &projectedCnfVars, const Map<Int, Float> &literalWeights) {
  ...
  template<typename T> Float adjustModelCount(Float apparentModelCount, const T &projectedCnfVars, Int totalLiteralCount) {
    Float totalModelCount = apparentModelCount;
